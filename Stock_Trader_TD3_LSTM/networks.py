...
class ActorNetwork(Network):
    def __init__(self, *args, num_steps=1, **kwargs):
        ...
        with graph.as_default():
            ...
            output2 = Dense(
                self.output_dim, activation=self.activation,
                kernel_initializer='random_normal')(output2)    
              
            self.model = Model(inp, output)
            self.target_model = Model(inp, output2)
            self.model.compile(
                optimizer=SGD(lr=self.lr), loss=self.loss)
    ...
    def get_network_head(inp):
        ...
        return Model(inp, output)
    @staticmethod
    def get_network_head2(inp):
        output2 = LSTM(256, dropout=0.1,
                      return_sequences=True, stateful=False,
                      kernel_initializer='random_normal')(inp)
        output2 = BatchNormalization()(output2)
        output2 = LSTM(128, dropout=0.1,
                      return_sequences=True, stateful=False,
                      kernel_initializer='random_normal')(output2)
        output2 = BatchNormalization()(output2)
        output2 = LSTM(64, dropout=0.1,
                      return_sequences=True, stateful=False,
                      kernel_initializer='random_normal')(output2)
        output2 = BatchNormalization()(output2)
        output2 = LSTM(32, dropout=0.1,
                      stateful=False,
                      kernel_initializer='random_normal')(output2)
        output2 = BatchNormalization()(output2)
        return Model(inp, output2)
