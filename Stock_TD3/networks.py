    ...
    def predict2(self, sample):
        with self.lock:
            with graph.as_default():
                if sess is not None:
                    set_session(sess)
                return self.model2.predict(sample).flatten()
...
class ActorNetwork(Network):
    def __init__(self, *args, num_steps=1, **kwargs):
        ...
        with graph.as_default():
            ...
            self.target_model1.set_weights(weights_)
    ...
    def train(self,x, y_poilcy):
        x = np.array(x).reshape((-1, self.num_steps, self.input_dim))

        actor_loss = -tf.reduce_mean(y_poilcy)

        self.actor_loss =super().train_on_batch(x, actor_loss)
...
class CriticNetwork(Network):
    ...
    def predict2(self, sample):
        sample = np.array(sample).reshape(
            (1, self.num_steps, self.input_dim))
        return super().predict2(sample)

    def train_on_batch(self, x, y):
        x = np.array(x).reshape((-1, self.num_steps, self.input_dim))
        return super().train_on_batch(x, y)

    def train(self,x,q1_values, q2_values,target):
        """Train Q-network for critic on sampled batch
        		"""
        x = np.array(x).reshape((-1, self.num_steps, self.input_dim))

        critic_loss_1 = tf.reduce_mean(tf.math.square(q1_values - target))
        critic_loss_1 = super().train_on_batch(x, critic_loss_1)


        critic_loss_2 = tf.reduce_mean(tf.math.square(q2_values - target))
        critic_loss_2 = super().train_on_batch(x, critic_loss_2)
